...
\end{align*}
where $\ell(m, z)= m - z\log m$ is the Poisson loss. The penalty term
measures the number of changepoint variables $c_t$ which are equal to
1 (which is the same as the number of peaks in the resulting model).
