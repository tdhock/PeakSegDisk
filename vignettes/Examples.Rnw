...
\item[PeakSegPipeline],
  ...
  for genome-wide peak calling using the other PeakSeg packages. (work
  in progress)
\end{description}
...
To run the peak detection algorithm a numeric penalty parameter must
be specified by the user. The smallest value is 0 which yields max
peaks, and the largest value is Inf which yields no peaks. The code
below runs the peak detection algorithm on this count data vector,
using the penalty parameter $\lambda = 10.5$:

<<>>=
(fit <- PeakSegDisk::PeakSegFPOP_vec(z.rep.vec, 10.5))
@

Given a sequence of $N$ non-negative count data
$z_1, \dots, z_N\in\mathbb Z_+$, the peak detection algorithm computes
...
The model output list above includes \verb|fit$segments|, a data table with
one row for each segment mean, and \verb|fit$loss|, a data table with one
...
\item \verb|equality.constraints|, the number of adjacent segment means that are equal in the optimal solution,
...
\Sexpr{fit$loss$bases} elements but the coverage.bedGraph data file
contains only \Sexpr{fit$loss$bedGraph.lines} lines. In real genomic data
...
chromEnd-chromStart). The optimization problem we solve is
...
\mathbf c\in\{-1, 0,1\}^{n-1}\\
  ...
  \sum_{i=1}^n w_i \ell(m_i, z_i) + \lambda \sum_{i=1}^{n-1} I(c_i \neq 0) \\
  \text{subject to\ \ } &\ \text{no change: }c_t = 0 \Rightarrow m_t = m_{t+1}\text{ and }s_t=s_{t+1}
  \nonumber\\
&\ \text{go up: }c_t = 1 \Rightarrow m_t \leq m_{t+1}\text{ and }(s_t,s_{t+1})=(0,1),
  \nonumber\\
...
Note that this function does not perform run-length encoding for you:
...
<<>>=
(fit.prob <- PeakSegDisk::PeakSegFPOP_dir(data.dir, 10.5))
@

Again the results above are the same as in previous computations.  The
underlying C++ code
