...
TODO type optimization problems.
...
* and then running `PeakSegFPOP_dir`.

The reason why this method is recommended for large data is because
`PeakSegFPOP_dir` saves its results to the "problem" directory. So
...
To use `PeakSegFPOP_dir` the data should be saved to a
...
`chrXX-start-end/coverage.bedGraph_penalty=0.1_loss.tsv` which
are used to store/cache the results. 
If the files already exist (and
...
Again the results above are the same as in previous computations.

### Computing the model with a given number of peaks

The `sequentialSearch_dir` function can be used to compute the
